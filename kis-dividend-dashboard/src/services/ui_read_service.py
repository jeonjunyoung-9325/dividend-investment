...
from src.services.dividend_forecast_service import ForecastTaxRates, forecast_rows
...
def get_dashboard_view() -> DashboardView:
    ...
    forecast_gross = _sum_column(dividends.forecast, "예상 세전")
    forecast_net = _sum_column(dividends.forecast, "예상 세후")
    metrics = (
    ...
    monthly = _monthly_frame(payments)
...
def _krw_payment(payment: DividendPayment) -> Decimal:
    if payment.net_amount_krw is not None:
        return payment.net_amount_krw
    return payment.net_amount if payment.currency == "KRW" else Decimal(0)
...
def _monthly_frame(payments: tuple[DividendPayment, ...]) -> pd.DataFrame:
    current = datetime.now(UTC).year
    values = {month: Decimal(0) for month in range(1, 13)}
    for payment in payments:
        if payment.payment_date.year == current:
            values[payment.payment_date.month] += _krw_payment(payment)
    return pd.DataFrame(
        {
            "월": [f"{month}월" for month in values],
            "실제": list(values.values()),
            "확정": [Decimal(0)] * 12,
            "예상": [Decimal(0)] * 12,
        }
    )
