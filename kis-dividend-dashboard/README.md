...
`gross_amount`, `tax_amount`, `net_amount`로 모두 명확하게 반환하는 전용 API는 찾지 못했습니다.
다만 국내 `기간별계좌권리현황조회`의 계좌별 현금 지급일·최종배정금액·세금·기준일 수량을
공식 KSD 배당일정의 종목·기준일·지급일·주당금액과 교차 대조할 수 있습니다. 이 앱은 두 공식
자료가 정확히 일치하고 세금이 0이라 gross/net 해석 차이가 없는 행만 실제 배당으로 자동
저장합니다. 세금이 있거나 대조되지 않는 행은 임의 해석하지 않습니다. 해외
...
1. 공식 계좌 권리와 공식 배당일정이 정확히 대조된 세금 0원 지급
2. 사용자가 한국투자증권에서 내려받아 업로드한 CSV 또는 XLSX
3. 사용자가 직접 입력한 배당 내역
4. 검증 가능한 외부 제공자 데이터
5. 데이터 없음 또는 계산 불가 표시

해외주식·미니스탁의 실제 입금 gross/tax/net은 전용 공식 API가 확인되지 않아 파일 가져오기나
직접 입력이 필요합니다. 파일 업로드 값은 `source`로 공식 API 값과 구분하고 파일·행 hash로
중복을 방지합니다. 확정
