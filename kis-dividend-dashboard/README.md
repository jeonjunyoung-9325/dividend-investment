...
공식 KSD 배당일정의 종목·기준일·지급일·주당금액과 교차 대조할 수 있습니다. 하지만 이 권리
자료만으로 계좌에 실제 입금된 세전·세금·실수령액을 확정할 수 없으므로 실제 배당으로 자동
저장하지 않습니다. 해외
...
1. 사용자가 한국투자증권에서 내려받아 업로드한 CSV 또는 XLSX
2. 사용자가 직접 입력한 배당 내역
3. 검증 가능한 외부 제공자 데이터
4. 데이터 없음 또는 계산 불가 표시

국내주식·해외주식·미니스탁의 실제 입금 gross/tax/net은 전용 공식 API가 확인되지 않아 파일
가져오기나 직접 입력이 필요합니다. 파일 업로드 값은 `source`로 구분하고 파일·행 hash로
중복을 방지합니다. 확정
